...
- **Color Grading & Custom Render Settings (Phase 7):** See `.agents/skills/daz-photography/SKILL.md`. Tools: `daz_apply_tone_mapping` (photographic tone-map: EV/Shutter/Aperture/ISO/Saturation/Gamma/Vignetting/Burn/Crush/White Point), `daz_apply_photographic_look(preset)` (9 named looks: golden_hour, moody_noir, high_key, low_key, vintage_film, fashion_editorial, cinematic_teal_orange, soft_portrait, dramatic_mono), `daz_set_environment_visibility` (Environment Mode / Draw Dome / Environment Intensity / Sun-Sky controls), `daz_apply_environment_look(preset)` (5 named: golden_hour, blue_hour, midday, overcast, night), `daz_set_advanced_iray_settings` (Max Samples, Render Quality, Max Time, Pixel Filter, Denoiser, Caustics), `daz_get_color_grade_settings` (snapshot). `daz_render_shot` accepts `tone_preset`, `env_preset`, `iray_samples`, `max_time` for one-shot graded renders.
- **Viewport Screenshot / Iterative Setup (Phase 7.1):** `daz_viewport_screenshot(output_path, width=480, height=270, camera, iray_samples, max_time, tone_preset, env_preset, critique, critique_model="gpt-4o-mini")` — fast low-res Iray preview for iterative scene setup. When `gpt_api_key` is set, returns a < 400-token GPT vision critique so the AI can refine the scene without ever loading the image. Default model **gpt-4o-mini** (~$0.0002 per 480×270 critique); pass `critique_model="gpt-4o"` for higher quality. Plus `daz_set_viewport_draw_style`, `daz_get_viewport_draw_style`, `daz_critique_viewport`. See `.agents/skills/daz-render-workflow/SKILL.md` "Iterative scene setup with viewport screenshots".
